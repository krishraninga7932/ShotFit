...
import {fetchDiet,addDiet,deleteDiet,updateDiet} from "../../Redux/adddietSlice"
...
const Admindiet = () => {
  ...
  const handleaddDiet = async (e) => {
    ...
    if (editId) {
      dispatch(updateDiet({ id: editId, updated: newDiet })).then(
        () => {
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-[#151515] p-6 rounded-xl shadow-lg w-[100%] max-w-xl mx-auto relative">
            ...
            <h2 className="text-xl font-semibold mb-4 text-white">
              {editId ? "Edit Diet" : "Create New Diet"}
            </h2>

            <form className="space-y-4" onSubmit={handleaddDiet}>
              <div>
                <label className="block text-white mb-1">Diet Name</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              >
                {editId ? "Update Diet" : "Create Diet"}
              </button>
            </form>
          </div>
        </div>
      ...
      <div>
        ...
        {list && list.length > 0 ? (
            ...
            {list.map((sup, index) => (
              ...
              >
                ...
                <div className="flex justify-end gap-3">
                  <button
                    onClick={() =>
                      dispatch(deleteDiet(sup.id)).then(() => {
          ...
          <p className="text-gray-400 text-center mt-10">
            No Diet added yet. Click “Add Diet to create one.
          </p>
        )}
      </div>
    </div>
